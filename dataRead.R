...
d_urls <- data.frame(file_name = links[grep("daily_aqi_by_county", links$url),])
d_urls <- rbind(d_urls,data.frame(file_name  = links[grep("hourly_WIND", links$url),]))
...
for (row in 1:nrow(d_urls)) {
  ...
  download.file(link,file.path("data",basename(link)), method = "libcurl", quiet = TRUE)
  }
